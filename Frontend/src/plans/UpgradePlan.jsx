...
function UpgradePlanCard() {
    ...
    return (
        <Link
            ...
            className={`
                relative flex items-center overflow-hidden transition-all duration-500 group
                ${!isNavMinimized
                    ? "gap-4 p-4 rounded-[2rem] mx-2 mb-4"
            ...
            `}
        >
            {/* Background Glow Effect (Dark Mode special) */}
            {isDarkMode && (
            ...
            <div className={`
                ...
                ${!isNavMinimized ? "w-12 h-12" : "w-10 h-10"} 
                bg-gradient-to-br from-blue-600 to-indigo-600 shadow-lg shadow-blue-500/30
                group-hover:scale-110
            `}>
                <i className={`ri-vip-crown-2-line ${!isNavMinimized ? "text-xl" : "text-lg"} text-white`}></i>
            </div>

            {/* Content Section (Only visible when Sidebar is expanded) */}
            {!isNavMinimized && (

                storeUserData?.userIs !== "pro" ?
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                            <h3 className={`text-[11px] font-black uppercase tracking-widest ${isDarkMode ? "text-blue-400" : "text-blue-600"}`}>
                                Upgrade Plan
                            </h3>
                        </div>

                        <p className={`text-xs font-bold truncate mt-0.5 ${isDarkMode ? "text-white/80" : "text-gray-800"}`}>
                            Get <span className="italic">Unlimited</span> Space
                        </p>

                        {/* Minimalist CTA */}
                        <div className="flex items-center gap-1 mt-2 text-[10px] font-black uppercase tracking-tighter text-blue-500 opacity-80 group-hover:opacity-100">
                            Explore PRO
                    ...
                    : <div>
                        <div className="flex items-center gap-2">
                            <h1 className={`text-[11px] font-black uppercase tracking-widest ${isDarkMode ? "text-blue-400" : "text-blue-600"}`}>
                                Manage Plan
                            </h1>
                        </div>
                    </div>
